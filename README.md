...
The steps below install the open-source TAPA compiler locally from source.
...
Run the visualizer locally from the repository root:

```bash
bazel run //tapa-visualizer:tapa-visualizer-devserver
```

Then open the local URL printed by the dev server and load the `graph.json`
file generated by TAPA from your work directory.
